...
class MediaInfo:
    ...
    def __init__(self, media_file):
        ...
        output = subprocess.Popen(
            infocmd, stdout=subprocess.PIPE).communicate()[0]
        prefix = ""
...
class VideoConverter:
    ...
    def _get_convert_commands(self, original, output):
        ...
        padscale = (
            "scale=%s:%s,pad=%s:%s:(ow-iw)/2:(oh-ih)/2:black" %
            (int(resize_width), int(resize_height), int(
                self.max_width), int(
                self.max_height)))
    ...
    def convert_video(self):
        ...
        if len(vparts) == 2:
            try:
                ...
                # convert the original video into each of the different output
                # formats
...
def main(argv):
    ...
    try:
        ...
        opts, args = getopt.getopt(argv, "hi:t:p:f:debv:g:m:", ["help", "input-dir=",
                                                                "output-dir=", "output-prefix=", "file=", "dry-run",
                                                                "exists", "backup",
